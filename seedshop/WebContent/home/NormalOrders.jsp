...
<div id="wrap"
	...
	try {
		...
		//주문 씨앗 목록
		String normalSeedOrders = "select distinct onlineorder.orderid, to_char(onlineorder.orderdate,'yyyymmdd'), nvl(seed.seedid,0), nvl(seed.seedname,'없음'), nvl(seedcompany.company,'없음'), onlineorder.saleprice , (select paidcheck from paid where paid.orderid = onlineorder.orderid), (select to_char(startdate,'yyyy-mm-dd') from delivery where delivery.orderid = onlineorder.orderid), (select to_char(enddate,'yyyy-mm-dd') from delivery where delivery.orderid = onlineorder.orderid) from onlineorder, normalcustomer, importantcustomer, seed, plant, seedcompany where onlineorder.seedid= seed.seedid and  seed.seedid = seedcompany.seedid and onlineorder.seedid = seedcompany.seedid and onlineorder.normalcustid =normalcustomer.normalcustid and  normalcustomer.normalcustid= "+ normalcustid +" order by onlineorder.orderid";
		
		pstmtSelect = conn.prepareStatement(normalSeedOrders);
		...
		String normalPlantOrders = "select distinct onlineorder.orderid, to_char(onlineorder.orderdate , 'yyyymmdd'), nvl(plant.plantid,0), nvl(plant.plantname,'없음'), nvl(plantcompany.company,'없음'), onlineorder.saleprice ,(select paidcheck from paid where paid.orderid = onlineorder.orderid), (select to_char(startdate,'yyyy-mm-dd') from delivery where delivery.orderid = onlineorder.orderid), (select to_char(enddate,'yyyy-mm-dd') from delivery where delivery.orderid = onlineorder.orderid) from onlineorder, normalcustomer, importantcustomer, plant, plantcompany where onlineorder.plantid= plant.plantid and  plant.plantid = plantcompany.plantid and onlineorder.plantid = plantcompany.plantid and onlineorder.normalcustid =normalcustomer.normalcustid and  normalcustomer.normalcustid= " + normalcustid +  "order by onlineorder.orderid ";
		
		pstmtSelect2 = conn.prepareStatement(normalPlantOrders);
	...
	<form action="index.jsp">
		<table style="margin: auto; width: 70%;">
			...
			<%
				// 주문 씨앗 입력
				while (rs.next()) {
						//씨앗이냐 모종이나 if문써서 하면되지 값이 0 이면 없는거니깐 번호가
						orderid = rs.getInt(1);
